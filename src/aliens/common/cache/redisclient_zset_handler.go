...
//删除多个
func (this *RedisCacheClient) ZRems(key string, member []interface{}) bool {
	conn := this.pool.Get()
	defer conn.Close()
	params := append([]interface{}{key}, member...)
	_, err := conn.Do(OP_Z_REM, params...)
	if err != nil {
		log.Debug("%v",err)
		return false
	}
...
func (this *RedisCacheClient) ZRevRangeByScoreBeforeLimit(key string, max int32, offset int32, count int32) []string {
	conn := this.pool.Get()
	defer conn.Close()
	result, err := redis.Strings(conn.Do(OP_Z_REVRANGEBYSCORE, key, max, PARAM_Z_MIN, PARAM_LIMIT, offset, count))
	if err != nil {
		//log.Debug("%v",err)
	}
	return result
...
func (this *RedisCacheClient) ZAll(key string) []interface{} {
	conn := this.pool.Get()
	defer conn.Close()
	result, err := conn.Do(OP_Z_RANGE, key, 0, -1)
	if err != nil {
		//log.Debug("%v",err)
		return nil
	}
...
func (this *RedisCacheClient) ZScore(key string, member interface{}) int64 {
	conn := this.pool.Get()
	defer conn.Close()
	result, err := redis.Int64(conn.Do(OP_Z_SCORE, key, member))
	if err != nil {
		//log.Debug("%v",err)
		return -1
	}
...
func (this *RedisCacheClient) ZCount(key string, min int32, max int32) int {
	conn := this.pool.Get()
	defer conn.Close()
	result, err := redis.Int(conn.Do(OP_Z_COUNT, key, min, max))
	if err != nil {
		return -1
	}
	return result
